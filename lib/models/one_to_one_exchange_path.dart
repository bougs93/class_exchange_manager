...
class OneToOneExchangePath implements ExchangePath {
  ...
  /// JSON 역직렬화 (로드용)
  ///
  /// JSON에서 OneToOneExchangePath를 복원합니다.
  /// ExchangeOption은 재생성할 수 없으므로, 기본 옵션을 생성합니다.
  factory OneToOneExchangePath.fromJson(Map<String, dynamic> json) {
    final sourceNode = ExchangeNode.fromJson(
      json['sourceNode'] as Map<String, dynamic>,
    );
    final targetNode = ExchangeNode.fromJson(
      json['targetNode'] as Map<String, dynamic>,
    );
    ...
    final option = ExchangeOption(
      ...
      reason: json['description'] as String? ?? '교체',
    );
