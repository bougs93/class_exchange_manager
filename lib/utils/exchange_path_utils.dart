...
class ExchangePathUtils {
  /// 특정 타입의 경로들만 필터링
  static List<T> filterByType<T extends ExchangePath>(
  ...
  /// 특정 타입을 제외한 경로들 필터링
  static List<ExchangePath> excludeType<T extends ExchangePath>(
    List<ExchangePath> paths,
  ) {
  ...
  /// 특정 타입의 경로들만 제거
  static List<ExchangePath> removePaths<T extends ExchangePath>(
    List<ExchangePath> paths,
  ) {
  ...
  /// 특정 타입의 경로 개수 반환
  static int countPathsOfType<T extends ExchangePath>(
    List<ExchangePath> paths,
  ) {
