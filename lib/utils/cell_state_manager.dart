...
class CellStateManager {
  ...
  bool isExchangeableTeacher(String teacherName, String day, int period) {
    return _exchangeableTeachers.any(
      (teacher) =>
  ...
  bool isInCircularPath(String teacherName, String day, int period) {
    ...
    return _selectedCircularPath!.nodes.any(
      (node) =>
  ...
  bool isTeacherInCircularPath(String teacherName) {
    ...
    return _selectedCircularPath!.nodes.any(
      (node) => node.teacherName == teacherName,
    );
  }

  /// 선택된 1:1 경로에 포함된 셀인지 확인
  bool isInSelectedOneToOnePath(
    String teacherName, {
    String? day,
    int? period,
  }) {
  ...
  bool isInDualPath(String teacherName, String day, int period) {
    ...
    return _selectedDualPath!.nodes.any(
      (node) =>
          node.teacherName == teacherName &&
          node.day == day &&
          node.period == period,
