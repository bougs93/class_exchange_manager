...
class PersonalExchangeFilter {
  ...
  static List<ExchangeHistoryItem> filterExchangesForPersonalSchedule({
    ...
    AppLogger.exchangeDebug(
      '[PersonalExchangeFilter] 전체 교체 리스트: ${allExchanges.length}개',
    );
    ...
    AppLogger.exchangeDebug(
      '[PersonalExchangeFilter] 필터링 완료: ${filteredExchanges.length}개',
    );
  ...
  static bool _matchesWeekDates(
    ...
    // 결보강 계획서에서 날짜 정보 조회
    final absenceDateStr =
        substitutionPlanState.savedDates['${exchangeId}_absenceDate'] ?? '';
    final substitutionDateStr =
        substitutionPlanState.savedDates['${exchangeId}_substitutionDate'] ??
        '';

    // 날짜가 하나도 지정되지 않은 경우: 전체 적용 (true 반환)
    if (absenceDateStr.isEmpty && substitutionDateStr.isEmpty) {
      AppLogger.exchangeDebug(
        '[PersonalExchangeFilter] 교체 $exchangeId: 날짜 미지정 → 전체 적용',
      );
      return true;
    }
    ...
    if (absenceDateStr.isNotEmpty) {
      if (weekDateStrings.contains(absenceDateStr)) {
        AppLogger.exchangeDebug(
          '[PersonalExchangeFilter] 교체 $exchangeId: 결강일($absenceDateStr) 매칭',
        );
        return true;
      }
    }
    ...
    if (substitutionDateStr.isNotEmpty) {
      if (weekDateStrings.contains(substitutionDateStr)) {
        AppLogger.exchangeDebug(
          '[PersonalExchangeFilter] 교체 $exchangeId: 교체일($substitutionDateStr) 매칭',
        );
        return true;
      }
    }
    ...
    AppLogger.exchangeDebug(
      '[PersonalExchangeFilter] 교체 $exchangeId: 날짜 불일치 → 제외',
    );
    return false;
  }
}
