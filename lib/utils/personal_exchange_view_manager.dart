...
class PersonalExchangeViewManager {
  ...
  static Future<int> enableExchangeView({
    ...
    required dynamic
    dataSource, // PersonalTimetableDataSource 또는 TimetableDataSource
  }) async {
    if (filteredExchanges.isEmpty) {
      AppLogger.exchangeInfo(
        '[PersonalExchangeViewManager] 필터링된 교체 리스트가 비어있습니다',
      );
      return 0;
    }

    AppLogger.exchangeInfo(
      '[PersonalExchangeViewManager] 교체 뷰 활성화 시작: ${filteredExchanges.length}개 교체',
    );
    ...
    if (successCount > 0) {
      ...
      AppLogger.exchangeInfo(
        '[PersonalExchangeViewManager] 교체 뷰 활성화 완료 - $successCount/${filteredExchanges.length}개 적용',
      );
    }
  ...
  static Future<void> disableExchangeView({
    ...
    AppLogger.exchangeInfo(
      '[PersonalExchangeViewManager] 교체 뷰 비활성화 완료 - 원본 데이터로 복원',
    );
  }
  ...
  static bool _executeExchangeFromHistory(
    ...
    try {
      ...
      AppLogger.exchangeDebug(
        '[PersonalExchangeViewManager] 교체 실행: ${exchangePath.type}',
      );
      ...
      AppLogger.exchangeDebug(
        '[PersonalExchangeViewManager] 지원하지 않는 교체 타입: ${exchangePath.type}',
      );
      return false;
    } catch (e) {
  ...
  static bool _executeOneToOneExchange(
    ...
    try {
      ...
      AppLogger.exchangeDebug(
        '[PersonalExchangeViewManager] 1:1 교체 실행 중 오류: $e',
      );
      return false;
    }
  }
  ...
  static bool _executeCircularExchange(
    ...
    try {
      ...
      AppLogger.exchangeDebug(
        '[PersonalExchangeViewManager] 순환 교체 실행 중 오류: $e',
      );
      return false;
    }
  }
  ...
  static bool _executeDualExchange(
    ...
    try {
      ...
      AppLogger.exchangeDebug(
        '[PersonalExchangeViewManager] 2중 교체 실행: A(${exchangePath.nodeA.displayText}) ↔ B(${exchangePath.nodeB.displayText})',
      );
      ...
      AppLogger.exchangeDebug(
        '[PersonalExchangeViewManager] 2중 교체 1단계: ${exchangePath.node1.displayText} ↔ ${exchangePath.node2.displayText}',
      );
      ...
      AppLogger.exchangeDebug(
        '[PersonalExchangeViewManager] 2중 교체 2단계: ${exchangePath.nodeA.displayText} ↔ ${exchangePath.nodeB.displayText}',
      );
      ...
      AppLogger.exchangeDebug(
        '[PersonalExchangeViewManager] 2중 교체 완료: 2단계 모두 성공',
      );
      return true;
    } catch (e) {
      AppLogger.exchangeDebug(
        '[PersonalExchangeViewManager] 2중 교체 실행 중 오류: $e',
      );
      return false;
    }
  }
  ...
  static bool _executeSupplementExchange(
    ...
    try {
      ...
      AppLogger.exchangeDebug(
        '[PersonalExchangeViewManager] 보강 실행: ${targetNode.displayText} → ${sourceNode.displayText}',
      );
