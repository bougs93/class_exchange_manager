...
class SubstitutionPlanStorageService {
  ...
  Future<bool> saveSubstitutionPlanData(SubstitutionPlanState state) async {
    try {
      ...
      if (success) {
        AppLogger.info(
          '결보강 계획서 날짜 정보 저장 성공: ${state.savedDates.length}개 날짜, ${state.savedSupplementSubjects.length}개 보강 과목',
        );
      } else {
  ...
  Future<SubstitutionPlanState?> loadSubstitutionPlanData() async {
    try {
      ...
      AppLogger.info(
        '결보강 계획서 날짜 정보 로드 성공: ${state.savedDates.length}개 날짜, ${state.savedSupplementSubjects.length}개 보강 과목',
      );
