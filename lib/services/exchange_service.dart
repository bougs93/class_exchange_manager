...
class ExchangeService extends BaseExchangeService {
  ...
  bool performOneToOneExchange(
    ...
    try {
      ...
      TimeSlot? slot1s = findTimeSlot(
        teacher1,
        day1,
        period1,
        timeSlots,
      ); // 문유란의 월6교시 (원본)
      TimeSlot? slot2s = findTimeSlot(
        teacher2,
        day2,
        period2,
        timeSlots,
      ...
      TimeSlot? slot2t = findTimeSlot(
        teacher2,
        day1,
        period1,
        timeSlots,
      ); // 정영훈의 월6교시 (목적지)
      TimeSlot? slot1t = findTimeSlot(
        teacher1,
        day2,
        period2,
        timeSlots,
  ...
  bool performSupplementExchange(
    ...
    try {
      ...
      TimeSlot? sourceSlot = findTimeSlot(
        sourceTeacher,
        sourceDay,
        sourcePeriod,
        timeSlots,
      ...
      TimeSlot? targetSlot = findTimeSlot(
        targetTeacher,
        targetDay,
        targetPeriod,
        timeSlots,
  ...
  bool performCircularExchange(
    ...
    try {
      ...
      for (int i = 0; i < nodes.length - 1; i++) {
        ...
        final currentSlot = findTimeSlot(
  ...
  void logExchangeableInfo(List<Map<String, dynamic>> exchangeableTeachers) {
    ...
    AppLogger.teacherEmptySlotsInfo(
      '선택된 셀: $selectedTeacher 교사, $selectedDay요일, $selectedPeriod교시',
    );
