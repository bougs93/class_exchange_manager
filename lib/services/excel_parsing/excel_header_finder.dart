...
class ExcelHeaderFinder {
  ...
  static Map<String, dynamic> findTeacherHeader(Sheet sheet) {
    try {
      ...
      for (
        int row = 1;
        row <= ExcelServiceConstants.maxHeaderSearchRows;
        row++
      ) {
        // 각 행의 모든 열을 확인 (최대 50열까지)
        for (
          int col = 1;
          col <= ExcelServiceConstants.maxColumnsToCheck;
          col++
        ) {
          ...
          for (String keyword in teacherHeaderKeywords) {
            ...
            if (normalized == normalizedKeyword) {
              developer.log(
                '교사명 헤더를 $row행 $col열에서 찾았습니다: $keyword',
                name: 'ExcelHeaderFinder',
              );
  ...
  static Map<String, dynamic> findDayHeaders(Sheet sheet) {
    try {
      ...
      if (bestDayHeaders.length >= minDayCount) {
        ...
        developer.log(
          '요일 헤더를 $bestRow행에서 찾았습니다: $bestDayHeaders',
          name: 'ExcelHeaderFinder',
        );
        return {'row': bestRow, 'days': bestDayHeaders};
      }
  ...
  static Map<String, List<int>> findPeriodsByDay(
    ...
    try {
      ...
      for (String day in dayHeaders) {
        ...
        for (
          int col = dayStartCol;
          col < dayStartCol + ExcelServiceConstants.maxPeriodsToCheck;
          col++
        ) {
          String cellValue = ExcelParsingUtils.getCellValue(
            sheet,
          ...
          if (period != null &&
              period >= 1 &&
              period <= ExcelServiceConstants.maxPeriodsToCheck) {
        ...
        // 디버깅 로그
        developer.log(
          '$day요일에서 찾은 교시: $periods (시작열: $dayStartCol)',
          name: 'ExcelHeaderFinder',
        );
      }
  ...
  static int? findDataStartColumn(
    ...
    try {
      ...
      if (firstDayStartCol == null) {
        developer.log(
          '첫 번째 요일($firstDay)의 시작 열을 찾을 수 없습니다.',
          name: 'ExcelHeaderFinder',
        );
        return null;
      }
      ...
      for (
        int col = firstDayStartCol;
        col < firstDayStartCol + ExcelServiceConstants.maxPeriodsToCheck;
        col++
      ) {
        String cellValue = ExcelParsingUtils.getCellValue(
          sheet,
        ...
        if (period == 1) {
          developer.log(
            '데이터 시작 열을 찾았습니다: $col열 ($firstDay요일 1교시)',
            name: 'ExcelHeaderFinder',
          );
          return col;
        }
      ...
      developer.log(
        '첫 번째 요일($firstDay)에서 1교시를 찾을 수 없습니다.',
        name: 'ExcelHeaderFinder',
      );
      return null;
    } catch (e) {
