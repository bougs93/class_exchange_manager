...
> 작성일: 2026-08-26
> 최종 수정: 2026-09-01 (§10 1~8단계 구현 완료 — 날짜 기반 구조 전환 · 마이그레이션 없음 정책 반영)
> 상태: §1~§9 구현 완료 (반영 결과는 §8, 점검은 §9) · **§10 1~8단계 구현 완료**
> (남은 것: `savedDates` 제거·파생 `exchangeId` 정리, 대규모 시간표 성능 실측 — §10.10 참조)
...
| **8** | ✅ **완료 (2026-09-01)** — 전체 검증 + §10.9 리스크 재점검. 아래 「8단계 반영 결과」 참조 | `start_screen.dart` |
...
### 10.9 리스크 (2026-09-01 8단계 재점검 — 상태 표시 포함)

1. ✅ **해소** — **구 데이터의 조용한 유실**: 1단계에서 `schemaVersion` 가드 +
   `.v1.bak` 백업을 날짜 필드 추가와 같은 커밋에 넣었다. 다만 **8단계 점검에서
   "시작 시 안내"가 실제로는 연결되지 않은 채 남아 있는 것을 발견했다** —
   `ExchangeHistoryService.consumeLegacyDataNotice()`가 정의만 되고 아무 데서도
   호출되지 않아, 백업은 조용히 되지만 사용자에게는 스낵바 하나 뜨지 않았다.
   `start_screen.dart`의 `_loadSavedData()` 끝에서 소비하도록 8단계에서 마저 연결했다
2. ✅ **해소** — **날짜 없는 교체 건은 존재할 수 없게 한다**: `absenceDate`/`substitutionDate`를
   1단계부터 필수(non-null)로 선언했다. "날짜 미지정 교체" 상태 자체가 코드에 없다
3. ⚠️ **부분 해소 — 실측 없음**: `resolvedTimetableProvider`는 Riverpod의 의존성 기반
   재계산 방지를 그대로 물려받아 **같은 입력에 대해 다시 합성하지 않는다.** 다만
   이것이 CLAUDE.md의 "1초 미만" 요구를 실제로 만족하는지 **대규모 시간표(예: 80명대
   교사, 학기 전체 교체 수십 건)에서 실측하지는 않았다.** 다음 세션에서 체감 지연이
   보고되면 이 지점부터 확인한다
4. ✅ **문서화 완료**: `resolved_timetable_provider.dart`, `exchange_view_provider.dart`
   등 핵심 파일 주석에 "§1-6 공유는 폐기가 아니라 범위 축소"를 명시했다
5. ⚠️ **미해소 — 의도적으로 남겨둠**: `exchangeId` vs `item.id` 두 식별자 공존은
   여전하다(`substitution_plan_viewmodel.dart`의 `_generateExchangeId`). 이유는
   1·2단계와 동일하다 — **`savedDates`를 아직 제거하지 못했기 때문**(대체 UI인
   6단계의 주차 그룹핑은 이미 나왔지만, 결보강 탭의 날짜 입력 UI 자체는 아직
   `savedDates` 기반이다). `savedDates` 제거가 이뤄지는 시점에 함께 정리해야 한다 —
   이 프로젝트의 다음 단계로 미룬다

### 10.10 8단계 반영 결과 (2026-09-01) — 전체 검증

`flutter analyze` 이슈 없음, `flutter test` 188건 전체 통과. 1~7단계 전 과정의
정적 검증을 마지막으로 한 번 더 통과시켰고, §10.9 리스크 5건을 코드와 대조해
상태를 갱신했다(위 §10.9 참조 — 2건은 실제로는 미해결/부분 해결 상태였다).

**검증에서 실제로 발견해 고친 것**

"전체 검증"이 형식적 통과 확인에 그치지 않도록 §10.9 리스크 목록을 코드와
하나씩 대조했고, 그 결과 **리스크 1("구 데이터 안내")이 말로는 해소되어
있었지만 실제로는 연결되지 않은 채 방치되어 있었다** — `consumeLegacyDataNotice()`가
정의만 되고 어디서도 호출되지 않아, 구 버전 데이터를 백업한 뒤에도 사용자에게는
아무 안내가 뜨지 않았다. 6단계에서 발견한 "완성됐지만 연결 안 된 코드" 패턴이
똑같이 반복된 것이다 — `start_screen.dart`의 `_loadSavedData()` 끝에서 소비하도록
연결해 스낵바 안내가 실제로 뜨게 했다.

**실행 검증의 한계**

이번 세션에서는 다른 창(Orca IDE)이 앱 창을 화면상 완전히 가리고 있었고, 이
환경의 컴퓨터 제어 도구가 창을 프로그래밍적으로 앞으로 가져오는 기능을 지원하지
않아(`capabilities.windows.focus: false`) **순환·2중 교체의 실제 클릭 실행까지는
재검증하지 못했다.** 잘못된 창을 클릭할 위험이 있어 시도를 중단했다 — 대신:

- 순환·2중 교체의 `CellMove` 분해 로직은 `resolved_week_test.dart`의 전용 유닛
  테스트(4a단계, 5건)로 검증되어 있다
- 1:1 교체 실행·주차 전환·되돌리기·일괄 출력·교체불가 재로드는 4·6·7단계에서
  이미 실제 앱으로 검증했다(각 단계의 「실행 검증」 절 참조)

순환·2중 교체의 **실제 클릭 실행**은 다음 세션에서 창 포커스 문제가 없을 때
한 번 더 확인하는 것을 권한다.
